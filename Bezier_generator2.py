...
class Bezier(nn.Module):
    def __init__(self, ps, ctps):
        ...
        self.x0 = ps[0, 0] #第一个横坐标
        self.x3 = ps[-1, 0]#最后一个横坐标
        self.y0 = ps[0, 1]
        self.y3 = ps[-1, 1]
        #inner_ps 去掉第一个和最后一个的中间的点
        self.inner_ps = torch.as_tensor(ps[1:-1, :], dtype=torch.float64)
        self.t = torch.as_tensor(np.linspace(0, 1, 81))#0-1取81个点
...
def train(x, y, ctps, lr):
    ...
    c=np.vstack(b)
    ps=c.transpose()
   # ps = np.vstack((x, y)).transpose()#vstack行顺序拼接 transpose转置
...
Mtk = lambda n, t, k: t**k * (1-t)**(n-k) * n_over_k(n,k)  #n_over_k返回n中取K的组合数量
BezierCoeff = lambda ts: [[Mtk(3,t,k) for k in range(4)] for t in ts]


def bezier_fit(x, y):
    dy = y[1:] - y[:-1]#前面去掉第一个 后面去掉最后一个
    dx = x[1:] - x[:-1]
    dt = (dx ** 2 + dy ** 2)**0.5
    t = dt/dt.sum()
    t = np.hstack(([0], t))#拼接数组
    t = t.cumsum()#每一项都变成之前数值得累加和

    data = np.column_stack((x, y)) #xy拼接成行列
    result=BezierCoeff(t)
    Pseudoinverse = np.linalg.pinv(result) # (9,4) -> (4,9)
   # Pseudoinverse = np.linalg.pinv(bezierCoeff(t))7
   # 4*7 内积 7*2(坐标)
    control_points = Pseudoinverse.dot(data)     # (4,9)*(9,2) -> (4,2)
    medi_ctp = control_points[1:-1,:].flatten().tolist()#flatten 折叠成一维数组
    return medi_ctp
...
def is_close_to_linev2(xs, ys, size, thres = 0.05):
        ...
        slopes = [(second.y-first.y)/(second.x-first.x) 
        
        if not (second.x-first.x) == 0.0 else math.inf*np.sign((second.y-first.y)) for first, second in zip(pts, pts[1:])]
...
for il, label in enumerate(labels):
    ...
    for iid, ddata in enumerate(data):
        ...
        curve_data_top = data[iid][0:lhc2].reshape(lhc4, 2)#top 取一半点修改lhc4行2列
        curve_data_bottom = data[iid][lhc2:].reshape(lhc4, 2)
        
        #获得了框选的四个点的坐标
        left_vertex_x = [curve_data_top[0,0], curve_data_bottom[lhc4-1,0]]#最左边的两个横坐标
        left_vertex_y = [curve_data_top[0,1], curve_data_bottom[lhc4-1,1]]#最左边的两个纵坐标
        right_vertex_x = [curve_data_top[lhc4-1,0], curve_data_bottom[0,0]]#最右边
    ...
    plt.clf()
# 注释
'''通过BezierCoeff把7个点换成了4个点，然后260行 取了81个点'''
#Question 
#